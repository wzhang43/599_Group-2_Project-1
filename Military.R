...
or.mil.G <- group_by(or.mil.2, MIL)
summarise(or.mil.G, n(), avg_inc = mean(PINCP), sd_inc=sd(PINCP), 
...
ggplot(or.mil.G, aes(PINCP))+geom_histogram(aes(y=..count../sum(..count..)))+facet_grid(MIL ~ ., scales="free")
...
## the median is teh same for all of them, and the average is approximately the sam3, between 18 & 19
